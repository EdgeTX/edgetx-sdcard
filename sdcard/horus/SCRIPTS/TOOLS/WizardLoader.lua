...
local function run(event, touchState)
  ...
  elseif (event == EVT_VIRTUAL_NEXT) or
         (event == EVT_VIRTUAL_NEXT_PAGE) then
  ...
  elseif (event == EVT_VIRTUAL_PREV) or
         (event == EVT_VIRTUAL_PREV_PAGE) then
    selWizard = selWizard - 1
    if (selWizard > numOfWizards) then
      selWizard = 1
    elseif (selWizard < 1) then
  ...
  elseif (event == EVT_VIRTUAL_ENTER) or 
         (event == EVT_VIRTUAL_ENTER_LONG) then
    return launchWizard()
  elseif (event == EVT_TOUCH_BREAK) or
         (event == EVT_TOUCH_TAP) then
    touchX = touchState.x
    touchY = touchState.y
