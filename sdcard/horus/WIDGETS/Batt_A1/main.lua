...
-- Widget to display the levels of lipo/non-lipo battery from single analog source (like the VFAS/A1/A2/mini-quad-flight-controller)
-- Offer Shmuely
-- Date: 2021
-- ver: 0.1
...
local _options = {
  ...
  { "Color", COLOR, YELLOW },
  { "Is_Lipo", BOOL, 1 } -- 0=show the voltage as is, 1=assume lipo, auto detect cell count, display as single cell
}
...
local function create(zone, options)
  ...
  wgt.options.Is_Lipo = wgt.options.Is_Lipo % 2 -- modulo due to bug that cause the value to be other than 0|1
...
local function calculateBatteryData(wgt)
  ...
  -- mainValue
  if wgt.options.Is_Lipo == 1 then
    wgt.mainValue = wgt.vCellLive
    wgt.secondaryValue = wgt.vTotalLive
  elseif wgt.options.Is_Lipo == 0 then
...
local function refresh(wgt, event, touchState)
  ...
  if (wgt.options.Is_Lipo == nil) then
    return
  end
