...
local function refreshZoneSmall(wgt)
    ...
    -- write text
    lcd.drawText(myBatt.x + 15, myBatt.y + 2, string.format("%2.2fV", wgt.mainValue), MIDSIZE + wgt.text_color + wgt.shadowed + wgt.no_telem_blink)
    lcd.drawText(myBatt.x + 15, myBatt.y + (myBatt.h /2) + 2, string.format("%2.0f%%", wgt.cellPercent), MIDSIZE + wgt.text_color + wgt.shadowed + wgt.no_telem_blink)
end

--- Zone size: 180x70 1/4th  (with sliders/trim)
--- Zone size: 225x98 1/4th  (no sliders/trim)
local function refreshZoneMedium(wgt)
    log("refreshZoneMedium()")
    local myBatt = { ["x"] = 0, ["y"] = 0, ["w"] = wgt.zone.w - 20, ["h"] = 35, ["segments_w"] = 15, ["color"] = WHITE, ["cath_w"] = 6, ["cath_h"] = 20 }

    -- draw values
    lcd.drawText(wgt.zone.x, wgt.zone.y + 30, string.format("%2.2fV", wgt.mainValue), DBLSIZE + wgt.text_color + wgt.shadowed + wgt.no_telem_blink)
    ...
    local cellW = 59
    for i = 1, wgt.cellCount, 1 do
        local cellY = wgt.zone.y + 70 + (i - 1) * (cellH - 1)
...
local function refreshZoneLarge(wgt)
    local myBatt = { ["x"] = 0, ["y"] = 70, ["w"] = 65, ["h"] = 121, ["segments_h"] = 30, ["color"] = WHITE, ["cath_w"] = 30, ["cath_h"] = 10 }
    ...
    -- draw cells
    local pos = { { x = 80, y = 90 }, { x = 80, y = 110 }, { x = 80, y = 130 }, { x = 80, y = 150 }, { x = 80, y = 170 }, { x = 80, y = 190 }, { x = 80, y = 210 }, { x = 80, y = 230 } }
    for i = 1, wgt.cellCount, 1 do
        local fill_color = getRangeColor(wgt.cellDataLive[i], wgt.cellMax, wgt.cellMax - 0.2)
        print(fill_color)
        log(string.format("fill_color: %d", fill_color))
...
local function refreshAppModeImpl(wgt, x, w, y, h)
    ...
    lcd.drawText(x + w, y + myBatt.y + 150, string.format("%2.2fV", wgt.mainValue), RIGHT + DBLSIZE + wgt.text_color + wgt.shadowed + wgt.no_telem_blink)
    lcd.drawText(x + w, y + myBatt.y + 190, wgt.cellPercent .. "%", RIGHT + DBLSIZE + wgt.text_color + wgt.shadowed + wgt.no_telem_blink)
    lcd.drawText(x + w, y + myBatt.y + 240, string.format("%2.2fV %dS", wgt.secondaryValue, wgt.cellCount), RIGHT + SMLSIZE + wgt.text_color + wgt.shadowed + wgt.no_telem_blink)
  ...
  local pos = { { x = 111, y = 110 }, { x = 164, y = 110 }, { x = 217, y = 110 }, { x = 111, y = 129 }, { x = 164, y = 129 }, { x = 217, y = 129 }, { x = 111, y = 148 }, { x = 164, y = 148 } }
    for i = 1, wgt.cellCount, 1 do
...
--- Zone size: 266x378 - app mode (full screen)
...
local function refresh(wgt, event, touchState)
    ...
    log(string.format("wgt.zone.w: %d, wgt.zone.h: %d", wgt.zone.w, wgt.zone.h))
    local t4 = getUsage();
    if (event ~= nil) then
    ...
    elseif wgt.zone.w > 130 and wgt.zone.h >  28 then refreshZoneSmall(wgt)
