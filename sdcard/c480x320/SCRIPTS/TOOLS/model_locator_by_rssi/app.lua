...
local function getSignalValues()
    ...
    if fieldinfo1 or fieldinfo2 then
        ...
        if v1 ~= 0 then
            v = v1
            txtUsing = "Using signal: ELRS 1RSS"
        else
            v = v2
            txtUsing = "Using signal: ELRS 2RSS"
        end
        if v == 0 then
            v = -115
        end
        return v, -115, 20, txPowerValue, txtUsing, "Set TX Power to 25mW Non-Dynamic"
    end
...
local function main(event, touchState)
    ...
    if txPower then
        lcd.drawText(3, 60, "Current TX Power: " .. tostring(txPower) .. "mW", ((txPower == targetTXPower1)or(txPower == targetTXPower2)) and DARKGREEN or RED)

        if txPower ~= targetTXPower1 and txPower ~= targetTXPower2 then
