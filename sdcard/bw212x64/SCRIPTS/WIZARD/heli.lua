...
local function runTypeConfig(event)
  lcd.clear()
  fields = TypeFields
  lcd.drawPixmap(123, 2,"img/heli/heli-type.bmp")
...
local function runStyleConfig(event)
  lcd.clear()
  lcd.drawPixmap(130, 2,"img/heli/heli-style.bmp")
  fields = StyleFields
...
local function runSwitchConfig(event)
  lcd.clear()
  lcd.drawPixmap(160, 7,"img/heli/heli-switch.bmp")
...
local function runThrConfig(event)
  lcd.clear()
  lcd.drawPixmap(118, 6,"img/heli/heli-throttle.bmp")
  fields = ThrFields
...
local function runCurveConfig(event)
  ...
  lcd.drawPixmap(138, 18,"img/heli/heli-curves.bmp")
  fields = CurveFields
...
local function runAilerConfig(event)
  lcd.clear()
  lcd.drawPixmap(130, 2,"img/heli/heli-ailer.bmp")
  fields = AilerFields
...
local function runEleConfig(event)
  lcd.clear()
  lcd.drawPixmap(107,23,"img/heli/heli-elev.bmp")
  fields = EleFields
...
local function runRudConfig(event)
  lcd.clear()
  lcd.drawPixmap(130, 2,"img/heli/heli-rud.bmp")
  fields = RudFields
...
local function runCreateModel(event)
  ...
  -- Gyro
  if TypeFields[1][5] == 0 then
    model.insertMix(4, 0,{source=88,name="Gyr",weight=25})
    model.setOutput(4,{name="TGa"})
  else
    model.insertMix( 4, 0,{source=88,name="HH",weight=25})
    model.insertMix( 4, 1,{source=88,name="Rat",weight=-25,switch=gyRate-1,multiplex=2})
    model.setOutput(4,{name="TGa"})
  end
