...
local function drawProgressBar()
    ...
    lcd.drawRectangle(LCD_W - 130, 10, 80, 8)
    lcd.drawFilledRectangle(LCD_W - 129, 12, width, 6);
end
...
local function redrawFieldsPage()
    ...
    for index = 1, numberPerPage, 1 do
        ...
        if field[COL5_CURR_VAL] == nil and field[COL2_TYPE] ~= T_HEADER then
            ...
            if field[COL2_TYPE] == T_VALUE then
                lcd.drawText(valueCol, margin + spacing * index, tostring(field[COL5_CURR_VAL]) .. field[COL8_V_UNIT], attr)
            elseif field[COL2_TYPE] == T_COMBO then
                if field[COL5_CURR_VAL] > 0 and field[COL5_CURR_VAL] <= #(field[COL6_COMBO_KEYS]) then
                    lcd.drawText(valueCol, margin + spacing * index, field[COL6_COMBO_KEYS][field[COL5_CURR_VAL]], attr)
                end
            end
        end
    end
...
local function runInfoPageLine(y1, s1, s2)
    local X1 = 20
    local X2 = math.floor(LCD_W * 0.30)
...
local function runInfoPage(event, touchState)
    ...
    lcd.drawText(80, 50, "Switch Reminder", DBLSIZE)

    runInfoPageLine(100, "CH13:  Gain", "")
    runInfoPageLine(130, "CH14 =  +100", "=> stability disabled")
    runInfoPageLine(160, "CH14 =        0",  "=> wind rejection")
    runInfoPageLine(190, "CH14 =   -100",  "=> self level")
    runInfoPageLine(220, "CH16 =  +100", "=> panic mode (self level)")
    return 0
end
