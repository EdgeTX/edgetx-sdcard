...
local margin = 10
local spacing = 25
local numberPerPage = math.floor((LCD_H - 30) / 25)
local touch_d0 = 0
local is_gyro_enabled = 1
local valueCol = math.floor(LCD_W * 0.6)
...
local function drawProgressBar()
    ...
    lcd.drawRectangle(LCD_W - 130, 12, 80, 8, GREY)
    lcd.drawFilledRectangle(LCD_W - 129, 12, width, 6, GREY)
end
    ...
    for index = 1, numberPerPage, 1 do
        ...
        if field[COL7_IS_VISIBLE] == nil or field[COL7_IS_VISIBLE] == 1 then
            ...
            if field[COL4_CURR_VAL] == nil and field[COL2_TYPE] ~= T_HEADER then
                lcd.drawText(valueCol, margin + spacing * index, "---", attr)
            else
                if field[COL2_TYPE] == T_VALUE then
                    lcd.drawText(valueCol, margin + spacing * index, field[COL4_CURR_VAL] .. field[COL8_V_UNIT], attr)
                elseif field[COL2_TYPE] == T_COMBO then
                    if field[COL4_CURR_VAL] >= 0 and field[COL4_CURR_VAL] < #(field[COL5_COMBO_KEYS]) then
                        lcd.drawText(valueCol, margin + spacing * index, field[COL5_COMBO_KEYS][1 + field[COL4_CURR_VAL]], attr)
                    end
                end
            end
        end
        ...
        if index == 1 and is_gyro_enabled == 0 then
            lcd.drawText(LCD_W / 4, LCD_H / 2 - 20, "Gyro Disabled", DBLSIZE + GREY)
            lcd.drawText(20, LCD_H / 2 + 20, "Receiver operate as simple RX", DBLSIZE + GREY)
        end
...
local function runConfigPage(event, touchState)
    ...
    if is_gyro_enabled == 1 then
        if fields[2][COL4_CURR_VAL] ~= nil then
            ...
            lcd.drawBitmap(wingBitmaps[1 + fields[2][COL4_CURR_VAL]], 10, 120)
        end
        if fields[3][COL4_CURR_VAL] ~= nil then
            ...
            lcd.drawBitmap(mountBitmaps[1 + fields[3][COL4_CURR_VAL]], LCD_W / 3, 140)
        end
    end
...
local function drawReminderLine(y1, s1, s2, s3)
    local X1 = 20
    local X2 = math.floor(LCD_W * 0.29)
    local X3 = math.floor(LCD_W * 0.30)
...
local function runInfoPage(event, touchState)
    ...
    lcd.drawFilledRectangle(0,0, LCD_W, LCD_H, LIGHTWHITE);
    drawScreenTitle("FrSky S8R/S6R Gyro setup", page, #pages)
    lcd.drawText(80, 50, "Switches Reminder", DBLSIZE)

    drawReminderLine(100, "CH9:",  "Gain", "")
    drawReminderLine(130, "CH10:", "+100", "=> stability disabled")
    drawReminderLine(160, "CH10:", "0", "=> wind rejection")
    drawReminderLine(190, "CH10:", "-100", "=> self level")
    drawReminderLine(220, "CH12:", "+100", "=> panic mode")
    drawReminderLine(250, "CH12:", "x3 times:", "=> activate self-check ")
    return 0
end
