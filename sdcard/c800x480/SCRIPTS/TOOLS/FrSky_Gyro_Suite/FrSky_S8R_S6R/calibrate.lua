...
local function refreshPage(event)
    ...
    if (calibrationStep < 6) then
        ...
        lcd.drawBitmap(calibBitmaps[calibrationStep + 1], 40, 80, 200)
        ...
        for index = 1, 3, 1 do
            ...
            -- values
            lcd.drawText(LCD_W / 2 - 80, 100 + 35 * index, field[1] .. ":" .. v_current, COLOR_THEME_SECONDARY1)
            -- background rect
            lcd.drawRectangle(prog_m - prog_w - mark_w_half, 100 + 35 * index, prog_w*2+ mark_w_half *2, 24, BLACK);
            -- expected pos
            lcd.drawFilledRectangle(prog_m + x_pos_expected - mark_w_half +1, 100 + 35 * index +1, mark_w -2, 24 -2, LIGHTGREY);
            -- current pos (+ shade)
            lcd.drawFilledRectangle(prog_m + x_pos - mark_w_half +1 +2, 100 + 35 * index +1 +2, mark_w -4, 24 -4, GREY);
            lcd.drawFilledRectangle(prog_m + x_pos - mark_w_half    +2, 100 + 35 * index    +2, mark_w -4, 24 -4, bg_color);
            -- middle mark
            lcd.drawFilledRectangle(prog_m -1, 100 + 35 * index, 2, 24, BLACK);
        end

        if (is_all_v_align) then
            if calibrationState == 0 then
                lcd.drawFilledRectangle(LCD_W / 3, LCD_H - 60, 250, 35, GREEN);
            else
                lcd.drawFilledRectangle(LCD_W / 3, LCD_H - 60, 250, 35, ORANGE);
            end
            lcd.drawText(LCD_W / 3 + 10, LCD_H - 55, "Ready! press [Enter]")
        else
            lcd.drawFilledRectangle(LCD_W / 3, LCD_H - 60, 270, 35, BLACK);
            lcd.drawText(LCD_W / 3 + 10, LCD_H - 55, "Press [Enter] when 3 greens", WHITE)
        end
    else
        lcd.drawText(LCD_W / 3, 80, "Calibration completed", 0)
        lcd.drawBitmap(bitmap.open("img/done.bmp"), LCD_W / 3, 140)
        lcd.drawBitmap(bitmap.open("img/done.png"), LCD_W / 2, 100)
        lcd.drawText(LCD_W / 3, LCD_H - 60, "Hold [RTN] to exit", attr)
    end
